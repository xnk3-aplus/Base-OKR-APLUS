...
class OKRSheetGenerator:
    ...
    def generate_payload_data(self, users_data: List[Dict], cycle_name: str) -> List[List]:
        """
        Generate raw data payload (list of lists) for Google Sheets sync.
        Mirrors the layout of generate_excel.
        """
        # 1. Header Rows
        user_names = [u['name'] for u in users_data]
        if not user_names: user_names = ["Demo User"]
        
        # Row 1: Title
        # Format: ĐÁNH GIÁ OKRS - THÁNG {n}- Q{n}/{năm}
        ...
        title_str = f"ĐÁNH GIÁ OKRS - THÁNG {month} - Q{quarter}/{year}".upper()
        
        row1 = ["", "", ""] + [""] * len(user_names)
        row1[1] = title_str # Move to Column B (Index 1) for merging
        
        # Row 2: Headers
        row2 = ["TT", "Nội dung", "Tự chấm điểm"] + user_names
        ...
        # Track totals for each user
        user_totals = [0] * len(users_data)
        
        for tt, content, score_ref, style_type, key in data_structure:
            row = [tt, content, score_ref]
        ...
        # Total Row
        total_row = ["", "Tổng cộng OKR", ""]
        total_row.extend(user_totals)
        
        payload.append(total_row)
